 ...
 */
class RcloneClient(private val context: Context) {
    ...
    /** Unlock a password-encrypted rclone.conf for this process. */
    fun unlockConfig(configPassword: String) {
        rpc("config/unlock", JSONObject().put("configPassword", configPassword))
    }
    /** Dump the loaded rclone config as JSON. Requires [unlockConfig] for encrypted configs. */
    fun dumpConfigJson(): String = rpc("config/dump").toString()
    /** Restart rclone against the current config, unlock it, then dump decrypted config JSON. */
    fun unlockAndDumpConfigJson(configPassword: String): String {
        shutdown()
        RcloneBridge.shutdown()
        initialize()
        unlockConfig(configPassword)
        return dumpConfigJson()
    }
    ...
    fun copyFile(
        ...
        options: RcloneTransferOptions = RcloneTransferOptions(),
    ) {
        rpc("operations/copyfile", copyFileParams(srcRemote, srcPath, dstRemote, dstPath, options))
    }
    ...
    fun copyFileAsync(
        ...
        val params = copyFileParams(srcRemote, srcPath, dstRemote, dstPath, options)
        params.put("_async", true)
        return rpc("operations/copyfile", params).getLong("jobid")
    }

    private fun copyFileParams(
        srcRemote: String, srcPath: String,
        dstRemote: String, dstPath: String,
        options: RcloneTransferOptions,
    ): JSONObject {
        val params = JSONObject()
        ...
        val config = options.toConfigJson()
        if (config.length() > 0) params.put("_config", config)
        return params
    }
...
data class RcloneTransferOptions(
    val transferConcurrency: Int = 2,
    val verifyChecksums: Boolean = false,
    val resumeTransfers: Boolean = true,
)

fun RcloneTransferOptions.toConfigJson(): JSONObject {
    return JSONObject().apply {
        transferConfigEntries().forEach { (key, value) -> put(key, value) }
    }
}

fun RcloneTransferOptions.transferConfigEntries(): Map<String, Any> {
    return buildMap {
        put("Transfers", transferConcurrency.coerceIn(1, 8))
        if (verifyChecksums) put("Checksum", true)
        if (resumeTransfers) put("NoTraverse", true)
    }
}
