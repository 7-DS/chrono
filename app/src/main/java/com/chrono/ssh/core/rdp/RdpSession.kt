...
import com.chrono.ssh.core.rdp.FrameCallback
import com.chrono.ssh.core.rdp.FrameData
import com.chrono.ssh.core.rdp.MouseButton
import com.chrono.ssh.core.rdp.PointerCallback
import com.chrono.ssh.core.rdp.RdpClient
import com.chrono.ssh.core.rdp.RdpConfig
import com.chrono.ssh.core.rdp.RdpException
import com.chrono.ssh.core.rdp.SessionCallback
...
object RdpRuntimeConfigPolicy {
    fun validate(width: Int, height: Int, colorDepth: Int, port: Int) {
        require(width in 640..3840) { "RDP width must be between 640 and 3840." }
        require(height in 480..2160) { "RDP height must be between 480 and 2160." }
        require(colorDepth == 15 || colorDepth == 16 || colorDepth == 24 || colorDepth == 32) { "RDP color depth must be 15, 16, 24, or 32." }
        require(port in 1..65535) { "RDP port must be between 1 and 65535." }
    }
}
...
class RdpSession(
    ...
    fun start() {
        if (closed) return
        RdpRuntimeConfigPolicy.validate(width, height, colorDepth, port)
