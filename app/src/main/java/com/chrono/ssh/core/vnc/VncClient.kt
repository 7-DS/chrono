...
import com.chrono.ssh.core.vnc.io.CountingInputStream
import com.chrono.ssh.core.vnc.io.ThroughputTracker
import com.chrono.ssh.core.vnc.protocol.Handshaker
import com.chrono.ssh.core.vnc.protocol.Initializer
import com.chrono.ssh.core.vnc.rendering.Framebuffer
...
class VncClient(private val config: VncConfig) : Closeable {
    ...
    private val targetFps = VncRuntimeConfigPolicy.normalizeFps(config.targetFps)
    ...
    fun start(host: String, port: Int) {
        VncRuntimeConfigPolicy.validatePort(port)
        ...
        serverEventLoop = Thread({
            ...
            try {
                while (running) {
                    ...
                    when (messageType) {
                        0x00 -> {
                            val update = com.chrono.ssh.core.vnc.protocol.FramebufferUpdate.decode(input)
                            framebuffer.processUpdate(update)
                        }
                        0x01 -> {
                            val colorMap = com.chrono.ssh.core.vnc.protocol.SetColorMapEntries.decode(input)
                        ...
                        0x03 -> {
                            val cutText = com.chrono.ssh.core.vnc.protocol.ServerCutText.decode(input)
